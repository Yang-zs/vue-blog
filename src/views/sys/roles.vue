  ...
  <div class="users">
    <el-card>
      ...
      <el-table
        ...
        style="width: 100%;"
      >
        <el-table-column align="center" type="index" width="100px" label="序号">
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    async handleOpenRoleDialog(row) {
      ...
      const { menuIds } = await RolesApi.getRoleInfo(row.id)
      console.log(menuIds, 'getRoleInfo')
      this.menuIds = menuIds
      this.getTree()
    },
    // 获取用户个人菜单权限
    async getTree() {
      const { menus } = await RolesApi.getMenuNav()
      this.data = menus
    },
    handleSubmitRole() {
      console.log(11)
    ...
    // 获取列表数据
    async getUserList() {
      const { code, records, size, pages, total } = await RolesApi.getRolesList(
        this.tableParameter
      )
